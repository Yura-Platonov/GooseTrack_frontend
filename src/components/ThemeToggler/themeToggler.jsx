import { Button, Vector } from './themeToggler.styled';
import React, { useState } from 'react';
import { useTheme } from '../../hooks/useTheme';

export default function ThemeToggler() {
  const { theme, setTheme } = useTheme();
  const [showDarkButton, setShowDarkButton] = useState(theme === 'light');
  const [showLightButton, setShowLightButton] = useState(theme === 'dark');
    ...
    <div className="themetoggle">
      {showLightButton && (
        <Button variant="secondary" onClick={toggleTheme}>
          ...
          >
            <path
              d="M7.989 16c0 4.418 3.594 8.011 8.011 8.011s8.011-3.594 8.011-8.011-3.594-8.011-8.011-8.011c-4.418 0-8.011 3.594-8.011 8.011zM16 11.189c2.653 0 4.811 2.158 4.811 4.811s-2.158 4.811-4.811 4.811c-2.653 0-4.811-2.158-4.811-4.811s2.158-4.811 4.811-4.811zM14.397 27.2h3.2v4.8h-3.2v-4.8zM14.397 0h3.2v4.8h-3.2v-4.8zM-0.003 14.4h4.8v3.2h-4.8v-3.2zM27.197 14.4h4.8v3.2h-4.8v-3.2zM3.55 26.181l3.392-3.395 2.264 2.262-3.392 3.395-2.264-2.262zM22.784 6.95l3.395-3.395 2.262 2.262-3.395 3.395-2.262-2.262zM6.947 9.214l-3.395-3.395 2.264-2.262 3.392 3.395-2.261 2.262zM28.442 26.182l-2.262 2.262-3.395-3.395 2.262-2.262 3.395 3.395z"
              fill="#3E85F3"
            />
          </Vector>
        </Button>
      ...
      {showDarkButton && (
        <Button className="Button" variant="secondary" onClick={toggleTheme}>
          <Vector
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
